...
import { Divider, Form, Input, Switch, TimePicker } from "antd";
...
export default function SetAvailabilityPage() {
  ...
  const [upsertAvailability, { isLoading: isSaving }] =
    useUpsertAvailabilityMutation();
  ...
  const backendMap = useMemo(() => {
    return (
      Array.isArray(availabilitySchedule) ? availabilitySchedule : []
    ).reduce((acc, item) => {
  ...
  useEffect(() => {
    const values = days.reduce((acc, day) => {
      ...
      acc[`${key}StartTime`] = backendItem?.startTime
        ? dayjs(backendItem.startTime, TIME_FORMAT)
        : null;
      acc[`${key}EndTime`] = backendItem?.endTime
        ? dayjs(backendItem.endTime, TIME_FORMAT)
        : null;
      acc[`${key}MaxAppointment`] =
        backendItem?.maxAppointments != null
          ? String(backendItem.maxAppointments)
          : "";
  ...
  const onFinish = async (values) => {
    ...
    for (const day of days) {
      ...
      const formMax =
        formMaxRaw === undefined || formMaxRaw === null || formMaxRaw === ""
          ? null
      ...
      if (existing) {
        ...
        if (!isAvailable) {
          ...
          const maxAppToSend =
            existing.maxAppointments != null ? existing.maxAppointments : 0;
        ...
        const maxToUse =
          formMax != null
            ? formMax
            : existing.maxAppointments != null
            ? existing.maxAppointments
            : 0;

        // require start & end to be present when making available
        if (!startToUse)
          errors.push(
            `${day}: start time missing (existing: ${
              existing.startTime ?? "none"
            })`
          );
        if (!endToUse)
          errors.push(
            `${day}: end time missing (existing: ${existing.endTime ?? "none"})`
          );
        ...
        if (isAvailable) {
          ...
          if (!formStart)
            errors.push(
              `${day}: please select start time (use the TimePicker)`
            );
          if (!formEnd)
            errors.push(`${day}: please select end time (use the TimePicker)`);
    ...
    try {
      ...
      const errMsg =
        err?.data?.message || err?.message || "Failed to save availability";
      toast.error(errMsg);
    }
  };
          ...
          >
            <div className="mt-6">
              {days.map((day, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex flex-col md:flex-row items-center justify-between mb-4 py-4 px-4 rounded-lg border border-[#F0F2F3] bg-white"
                  >
                    {/* Left: Day label */}
                    <div className="w-1/6 min-w-[110px] text-center">
                      <div className="text-[#202020] font-medium text-[18px]">
                        {day}
                      </div>
                    </div>
                    ...
                    <div className=" text-center  lg:flex lg:gap-20 ">
                      <div className="lg:flex gap-4">
                        <div className="flex flex-col ">
                          <label className="text-[14px] text-[#848A8C] mb-2">
                            Set Start time
                          </label>
                      ...
                      <div className="flex flex-col ">
                        ...
                        <Form.Item
                          name={`${key}MaxAppointment`}
                          className="m-0"
                        >
                          <Input
                            size="large"
                            className=" !rounded-[6px] text-sm  !bg-[#E8EDF0] !border !border-gray-300 placeholder-[#9AA0A2]"
            ...
            <div className="mt-6 flex justify-center">
              <TealBtn
                ...
                disabled={isSaving}
                className="shadow-[0_10px_18px_rgba(20,74,108,0.28)]"
              />
            </div>
          </Form>
        </div>
