    ...
    return (
        <div className='p-2 bg-white border border-black/10 rounded-[10px] grid grid-cols-1 md:grid-cols-5 items-center gap-4'>
            ...
            <div className='flex flex-col md:flex-row gap-2 items-center md:items-stretch md:col-span-2'>
                {/* image */}
                <div className='w-30  '>
                    <Image src={booking?.service?.thumbnail} alt="image" className="rounded-[8px] w-full h-full object-cover" width={40} height={30}/>
                </div>

                {/* details */}
                <div className='space-y-2  text-center md:text-left '>
                    <h3 className='text-[#0A0A0A] font-open-sans font-semibold'>{booking?.service?.title}</h3>
                    {
                        role === 'USER' && (
                            <p className='bg-[#e7f3e9] rounded-md p-1 text-sm text-[#1e863a] font-nunito font-medium'>{booking?.service?.category?.title}</p>
                        )
                    }
